...
export class AppComponent {
  ...
  t=JSON.stringify(this.temp);
  books=JSON.parse(this.t);
  total:number=0;
  insertBook(e:Book)
  {
      let itemExists = false;
    for(let i=0; i<this.cartItems.length; i++){
      if(this.cartItems[i].b.ISBN === e.ISBN){
        itemExists = true;
        break;
      }
    }
    if(!itemExists){
      ...
      this.total = Math.round(this.total+ e.price.value)
      this.cartItems.push(cartItem);
    }
  }
  increaseCount(item:Cart)
  {
    item.quantity+=1;
    this.total=Math.round(this.total+item.b.price.value);
  }
  decreaseCount(item:Cart)
  {
    if(item.quantity==1)
    {
      this.removeItem(item.b.ISBN)
    }
    else
    {
      item.quantity-=1;
  ...
  removeItem(id:number){
    let itemIndex = this.cartItems.findIndex(obj => obj.b.ISBN === id)
    this.total = Math.round(Math.round(this.total) - Math.round(this.cartItems[itemIndex].b.price.value*this.cartItems[itemIndex].quantity))
